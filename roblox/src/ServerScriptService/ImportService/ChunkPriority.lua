...
local function makeMetrics(
    ...
    local resolvedCenterX, resolvedCenterZ = centerX, centerZ
    if resolvedCenterX == nil or resolvedCenterZ == nil then
        resolvedCenterX, resolvedCenterZ = getChunkCenterXZ(chunkLike, chunkSizeStuds)
    end

    local dx = resolvedCenterX - focusPoint.X
    local dz = resolvedCenterZ - focusPoint.Z
...
local function compareMetrics(aId, aMetrics, bId, bMetrics, forwardVector, observedCostById)
    ...
    if type(observedCostById) == "table" then
        local aObserved = observedCostById[aId] or 0
        local bObserved = observedCostById[bId] or 0
...
function ChunkPriority.BuildChunkPriorityKey(
    ...
    local metrics = makeMetrics(getChunkLike(chunkLikeOrEntry), focusPoint, chunkSizeStuds)
    ...
    return {
        ...
        observedCost = if type(observedCostById) == "table"
            then observedCostById[chunkId] or 0
            else 0,
...
function ChunkPriority.CompareChunkEntries(
    ...
    return compareMetrics(
        getChunkId(left),
        makeMetrics(getChunkLike(left), focusPoint, chunkSizeStuds),
        getChunkId(right),
        makeMetrics(getChunkLike(right), focusPoint, chunkSizeStuds),
        forwardVector,
        observedCostById
    )
end
...
function ChunkPriority.SortChunkIdsByPriority(
    ...
    for _, chunkId in ipairs(chunkIds or {}) do
        ...
        if chunkLike then
            metricsById[chunkId] = makeMetrics(chunkLike, focusPoint, chunkSizeStuds)
        end
    end
