...
local function estimatePolygonSurfaceY(chunk, worldPts)
    ...
    for _, point in ipairs(worldPts) do
        local groundY = GroundSampler.sampleWorldHeight(chunk, point.X, point.Z)
    ...
    local centroidGroundY =
        GroundSampler.sampleWorldHeight(chunk, sumX / #worldPts, sumZ / #worldPts)
            ...
            for _, hole in ipairs(water.holes) do
                if #hole >= 3 then
                    ...
                    for _, p in ipairs(hole) do
                        table.insert(
                            holePts,
                            Vector3.new(p.x + originStuds.x, cy, p.z + originStuds.z)
                        )
                    end
