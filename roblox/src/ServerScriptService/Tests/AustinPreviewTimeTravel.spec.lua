return function()
    ...
    local ok, err = xpcall(function()
        ...
        Assert.truthy(
            previewLoadCalls[1].options == nil,
            "expected live preview to use the built StudioPreview fixture"
        )
        ...
        Assert.equal(
            #fullLoadCalls,
            0,
            "expected request-driven preview mode not to load the canonical full-bake family"
        )
        Assert.equal(
            previewLoadCalls[1].indexName,
            AustinPreviewBuilder.FALLBACK_PREVIEW_INDEX_NAME,
        ...
        Assert.equal(
            ...
            "expected full-bake mode to load the canonical Austin manifest family"
        )
        ...
        Assert.equal(#previewLoadCalls, 1, "expected Clear to invalidate cached preview accelerator handle")
        ...
        Assert.equal(#previewLoadCalls, 1, "expected hard pause to reload the preview accelerator handle")
        Assert.truthy(previewLoadCalls[1].options.freshRequire == true, "expected hard pause to use fresh require")
        ...
        Assert.equal(
            #previewLoadCalls,
            1,
            "expected state-only preview epoch changes not to trigger a second preview accelerator load"
        )
