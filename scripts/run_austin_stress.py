...
EDIT_MARKERS = (
    ...
    "[AustinPreviewBuilder] sync complete",
...
def has_runtime_markers(lines: list[str]) -> bool:
    return any(any(marker in line for marker in RUNTIME_MARKERS) for line in lines)


def has_edit_markers(lines: list[str]) -> bool:
    return any(any(marker in line for marker in EDIT_MARKERS) for line in lines)
...
def parse_iteration(lines: list[str]) -> dict[str, Any]:
    result: dict[str, Any] = {
        ...
        "play": None,
        "play_late": None,
        "edit_markers": has_edit_markers(lines),
        "runtime_markers": has_runtime_markers(lines),
        "lines": lines,
    }
    for line in lines:
        ...
        if result["edit"] is None:
            payload = parse_json_marker("ARNIS_MCP_EDIT_ACTION", line)
            if payload is not None:
                result["edit_action"] = payload
                continue
        if result["edit"] is None:
...
def evaluate_iteration(iteration: dict[str, Any]) -> list[str]:
    failures: list[str] = []
    if not iteration.get("edit_markers"):
        failures.append("Edit markers missing")
    if not iteration.get("runtime_markers"):
        failures.append("Runtime markers missing")
